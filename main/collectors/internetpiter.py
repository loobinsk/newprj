...
logger = logging.getLogger('arendapiter')
...
class InternerPiterCollector(Collector):
            ...
            try:
                ...
                if elems:
                    ...
                    try:
                        ...
                        logger.info('получена площадь помещения %s' % advert.square)
                    except:
                        pass
                ...
                if elems:
                    ...
                    try:
                        ...
                        logger.info('получена жилая площадь помещения %s' % advert.living_square)
                    except:
                        pass
                ...
                if advert.adtype == Advert.TYPE_LEASE:
                    ...
                    if elems:
                        ...
                        if u'мес' in txt:
                            ...
                            logger.info('на длительный срок')
                ...
                if elems:
                    ...
                    if table:
                        ...
                        if u'Меб+' in txt:
                            ...
                            logger.info('с мебелью')
                        if u'Тв+' in txt:
                            advert.tv = True
                            logger.info('с телевизором')
                        if u'Хол+' in txt:
                            advert.refrigerator = True
                            logger.info('с холодильником')
                        if u'СтМ+' in txt:
                            advert.washer = True
                            logger.info('со стиральной машиной')
                        if u'Тел+' in txt:
                            advert.phone = True
                            logger.info('с телефоном')
                        if u'Инт+' in txt:
                            advert.internet = True
                            logger.info('с интернетом')
                    ...
                    try:
                        ...
                        if company_page:
                            ...
                            if company:
                                logger.info('получено агентство')
                    ...
                    try:
                        ...
                        if agent_page:
                            ...
                            if agent:
                                logger.info('Получен агент')
                    except Exception, err:
                        logger.info(traceback.format_exc())

                    if not agent:
                        logger.info('Агент не найден')
                        continue
                    else:
                        advert.user = agent
                        if not company:
                            ...
                            logger.info('Создана агенство для агента')
    ...
    def get_agent_from_href(self, href):
        ...
        logger.info( 'Агент с кодом %s' % agent_id)
        ...
        logger.info( 'загружена страница агента')
        ...
        if tbl:
            for tr in tbl[0].xpath('.//tr'):
                ...
                if u'Имя, Отчество' in tds[0].text_content():
                    ...
                    logger.info( 'получено фио агента')
                if u'Телефон' in tds[0].text_content():
                    agent.tel = clear_tel_list(tds[1].text_content()).encode('utf8')
                    logger.info( 'получен телефон агента')
                if u'Почта' in tds[0].text_content():
                    txt = tds[1].text_content().encode('utf8').split(',')
                    m1 = re.search('"(.+)"', txt[0])
                    m2 = re.search('"(.+)"', txt[1])
                    ...
                    logger.info( 'получен емайл агента %s' % agent.agent_email)
        exists_user = []
        if agent.tel:
                ...
                if exists_user:
                    ...
                    if logo:
                        try:
                            ...
                            logger.info( 'загружено фото агента')
                        except Exception, err:
                            logger.info( traceback.format_exc())

                    agent.gen_password()
                    logger.info( 'сгенерирован пароль')
                    agent.save()
    ...
    def collect(self):
        ...
        logger.info( 'Загрузка с сайта arenda-piter.ru')
    ...
    def collect_all(self):
        ...
        logger.info( 'Загрузка с сайта arenda-piter.ru всех объявлений')
