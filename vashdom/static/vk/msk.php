...
for ($i=0; $i<count($hot["user_id"]); $i++)
{
	...
	$sql = "INSERT INTO ca_msk (date, user_id, action, adress, type) VALUES ".$val;
	mysql_query($sql);
}
}
...
foreach ($bl as $black_list)
{
	...
	$sql = "INSERT INTO ca_msk (date, user_id, type) VALUES ".$val;
	mysql_query($sql);
}
}
...
$sql = "DELETE FROM ca_msk WHERE (date < DATE_SUB(NOW(), INTERVAL ".$days_to_delete." DAY)) AND (type = 'warm' OR type = 'hot')";
mysql_query($sql);

//Подсчет активности для занесения в черный список
$sql = "UPDATE ca_msk SET type='bl' WHERE user_id IN (SELECT user_id FROM (SELECT user_id, COUNT(*) AS total FROM ca_msk WHERE (type='warm' OR type='hot') GROUP BY user_id ORDER BY total DESC) AS counted_table WHERE counted_table.total >= ".$action_count_for_bl.")";
mysql_query($sql);

//Удаление повторов из черного списка
$sql = "DELETE main_table FROM ca_msk main_table, ca_msk clone_table WHERE main_table.user_id = clone_table.user_id AND main_table.type='bl' AND main_table.id > clone_table.id";
mysql_query($sql);

//Выгрузка горячих
$sql = "SELECT DISTINCT user_id FROM ca_msk WHERE (date > DATE_SUB(NOW(), INTERVAL ".$days_to_upload." DAY)) AND (type = 'hot') AND (user_id NOT IN (SELECT user_id FROM ca_msk WHERE type = 'bl'))";
...
file_put_contents("msk_hot.txt", $hot_str);

//Выгрузка теплых
$sql = "SELECT DISTINCT user_id FROM ca_msk WHERE (date > DATE_SUB(NOW(), INTERVAL ".$days_to_upload." DAY)) AND (type = 'warm') AND (user_id NOT IN (SELECT DISTINCT user_id FROM ca_msk WHERE (type = 'bl' OR type = 'hot')))";
$result = mysql_query($sql);
...
file_put_contents("msk_warm.txt", $warm_str);
